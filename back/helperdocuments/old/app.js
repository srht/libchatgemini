...
const chatModel = new ChatOpenAI({
  openAIApiKey: process.env.OPENAI_API_KEY,
  modelName: "gpt-4o-mini", // gpt-4 veya gpt-4o da kullanabilirsiniz
  temperature: 0.7,
});
...
app.post("/ask-chatbot", async (req, res) => {
  ...
  try {
    ...
    const questionAnsweringPrompt = PromptTemplate.fromTemplate(
      `Sen yardımcı bir kütüphane asistanısın. Kullanıcının sorusuna verilen BAĞLAMDAKİ bilgilere öncelik vererek cevap ver.  Yer bilgisi sorusuna kısaca cevap ver.
      Örneğin "bilgisayar kitapları" nerede gibi bir soru sorulursa, önce kendi genel bilginden konunun Library of Congress Sınıflandırmasını al ve BAĞLAMDA harfe göre yer bilgisi belirtildiyse, örneğin "Bilgisayar Bilimleri kitapları 3. katta bulunur." gibi bir yanıt ver
